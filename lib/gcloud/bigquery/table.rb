  ...
  module Bigquery
    ...
    class Table
      ...
      def link source_url, create: nil, write: nil, dryrun: nil
        ensure_service!
        options = { create: create, write: write, dryrun: dryrun }
        gapi = service.link_table table_ref, source_url, options
        Job.from_gapi gapi, service
      end
      ...
      def load_resumable file, options = {}
        chunk_size = Gcloud::Upload.verify_chunk_size options[:chunk_size],
                                                      file.length
        gapi = service.load_resumable table_ref, file, chunk_size, options
        Job.from_gapi gapi, service
      end

      def load_multipart file, options = {}
        gapi = service.load_multipart table_ref, file, options
        Job.from_gapi gapi, service
      end
